### V6.9.3 (2026-04-30)
- **說明書版本號自動同步**
  - 功能使用說明書.md 的版本號與日期在每次打包時由 sed 自動更新
  - 打包指令範本已更新，往後不需手動改說明書版號
...
- **PPTX 配色主題（整合自 MDT V4.6.0 的 HTML_THEMES）**
  ...
  - 主題以三色色票橫排顯示在 PPTX 按鈕左側，點擊即時切換
  - 選擇記憶在 localStorage（mdt_pptx_theme），重新進入保持上次選擇
  - PPTX 產出時自動套用選定主題的標題色 / 強調色 / 底色

### V6.9.0 (2026-04-28) — MDT Phase 1 新功能（整合自 MDT V4.6.2）
- **複製會議**
  - 會議詳情面板新增「複製」按鈕
  - 選擇目標日期後複製整場會議的病人清單
  - 複製內容：病人清單、治療記錄、影像結果
  - 清空內容：討論要點、結論、後續追蹤（讓個管師重新填寫）
- **會後填寫 Panel**
  - 會議室 Header 新增「會後填寫」按鈕（每場會議都可用）
  - 一次看所有討論個案，補填討論摘要 / 結論 / 後續追蹤計畫
  - 「帶入下次追蹤」toggle：關閉後該個案不會出現在下次 MDT 前期追蹤名單
  - 儲存時同步更新病人事件卡的 MDT 結論
- **MeetingCase 新增 followNext 欄位**（預設 true）
  - 前期追蹤自動名單現在尊重 followNext 設定
...
- **更新科別與醫師主檔（15科，共 38 位醫師）**
  ...
  - 耳鼻喉科補入鍾從得
  - 肝膽腸胃科補入王俊偉
- **MDT_GROUP_CONFIG 大幅強化**
  - 新增 location 欄位（每場 MDT 的實際地點）
  - 新增 members 欄位（每場 MDT 的固定參與醫師清單）
  - 新增 coMeeting 欄位（合場開會對應群組）
  - 血液淋巴癌（LY）從頭頸癌群組獨立為「血液淋巴癌」群組，個管師楊靜雯
  - 頭頸癌和血液淋巴癌維持合場開會，但各自有獨立記錄
- **新增血液淋巴癌（紫色系）的 MDT 頁面色彩**
- **CANCER_CODE_TO_MDT_GROUP：LY 對應更新為血液淋巴癌**
...
- **修復：換版後不需重置資料庫也能登入（帳號 migration）**
  ...
  - 舊版本有 nurse1/nurse2 帳號，但新版改成 14002/11750，造成帳號不存在、密碼錯誤
  - 修復：改為逐一檢查每個預設帳號是否存在，不存在才新增
  - 效果：直接換上新版刷新頁面，系統自動補建 14002/YANG、11750/KAO，無需重置資料庫
...
- **🔴 修復：DB 開啟失敗（資料庫初始化崩潰）**
  ...
  - Dexie 升級 v4→v5 時，v4 的 11 張表全部被刪除（只保留 users）
  - 任何有舊資料的瀏覽器升級都會崩潰
  - 修復：v5 stores() 補回所有 12 張表（v4 的 11 張 + users）
- **預設帳號調整**
  | 帳號 | 密碼 | 顯示名稱 | 負責癌別 |
  |------|------|---------|---------|
  | admin | admin1234 | 系統管理員 | 全部 |
  | nurse1 | KAO1234 | KAO | BC / LC / EC |
  | nurse2 | YANG1234 | YANG | CRC / GC / LY / CC / OVC / EMC |
...
- **新增兩個預設個管師帳號**
  ...
  |------|------|---------|---------|
  | admin | admin1234 | 系統管理員 | 全部 |
  | nurse1 | nurse1234 | 個管師（胸腔乳癌） | BC / LC / EC |
  | nurse2 | nurse1234 | 個管師（消化道血液） | CRC / GC / LY |

### V6.6.1 (2026-04-10)
- **🔴 修復：密碼錯誤（無法登入）**
  - 根本原因：DB Schema v6 缺少 `users` 表
  - Dexie 規則：新版本的 stores() 必須列出所有要保留的 table，未列出的會被刪除
  - v5 有 users，v6 忘了列 → 升級到 v6 時 users 表被刪除 → ensureDefaultAdmin() 找不到表 → 無帳號可登入
  - 修復：v6 的 stores() 補回 users 表（含注釋提醒）
  - 重要：因為 users 已被刪除，需要系統工具→重置資料庫，讓 v6 重新建立 users 表
...
- **修復：測試資料載入時沒有建立 MDT 會議記錄**
  - 根本原因：seedDefaultData 只讀 ALL_TEST_PATIENTS，沒有讀 TEST_MEETINGS
  - 修復：在病人載入完成後，直接在 seedDefaultData 內建立 3 場 MDT 測試會議和對應討論記錄
  - 第一場乳癌 MDT（已完成）：BC01（新個案）+ BC02（必要提報，切緣陽性+新輔助）
  - 第二場消化道癌 MDT（已完成）：GC01（新個案）+ CRC02（前期追蹤）
  - 第三場乳癌 MDT（草稿）：下次開會預備

### V6.5.8 (2026-04-10)
- **全面重新設計測試資料（55位病人 + 3場 MDT 會議）**
  - **MRN 格式改為 T- 開頭**（如 T-BC001、T-WC001），徹底解決無法刪除的問題
  - 14個癌別各 3 人（含新增的 LY 血液淋巴癌 3 人）= 42 位指標病人
  - 工作中心觸發病人 13 人（涵蓋所有觸發類型）
  - 3 場 MDT 測試會議含完整討論記錄：
    - 乳癌 MDT（已完成）：BC01 新個案 + BC02 必要提報，含影像/病理/結論/後續
    - 消化道癌 MDT（已完成）：GC01 新個案 + CRC02 前期追蹤
    - 乳癌 MDT（草稿）：下次開會預備
- **修復：刪除測試資料現在真的有效**
  - 刪除條件改為 mrn.startsWith('T-')
  - 同時刪除：測試病人 + 事件 + 醫師配對 + meetingCases + 所有 meetings

### V6.5.7 (2026-04-10)
- **Phase 4：MDT 產出功能完整實作**
  - 安裝 pptxgenjs + docx 函式庫
  - 新增 mdtOutputService.ts（PPTX / DOCX / LINE 產出服務）
  - **PPTX 簡報：**
    - 封面頁：會議標題、日期時間、地點、個管師、Zoom、個案總覽
    - 各 Section 封面頁：標籤 + 所有個案列表
    - 每位病人一頁：姓名/病歷號/年齡/癌別/分期、初診日、治療記錄（自動帶入）、影像結果、病理結果、討論要點、結論建議（有結論時綠色顯示）、後續追蹤
  - **DOCX 會議記錄：**
    - 完整會議資訊、所有 Section 內容、每位病人的完整討論記錄（table格式）
  - **LINE 通知文字：**
    - 民國年日期格式、遮蔽病人姓名（○字保護）
    - 各 Section 列出個案、影像/討論摘要（前50字）
    - 自動複製到剪貼簿，彈出預覽視窗可再次複製
  - 會議室 Header 右側新增三個產出按鈕（LINE/DOCX/PPTX）

### V6.5.6 (2026-04-10)
- **Phase 3：MDT 會議室完整實作**
  - 進入會議 → 會議室頁（MDTMeetingPage）
  - **5個Section 完整功能：**
    - Section 1 新個案：自動列出上次同癌別MDT後的新收病人
    - Section 2 前期追蹤：自動列出上次有結論、待追蹤的病人
    - Section 3 必要提報：自動偵測基準2.4八類（分期未確認/年齡≥85/新輔助/切緣陽性/第二原發/requiresMDT flag）
    - Section 4 醫療小組：記錄個管師回覆意見追蹤
    - Section 5 特殊議程：手動搜尋加入臨時病人
  - **病人資料自動帶入：** 治療摘要（手術/化療/放療/標靶/荷爾蒙），不需重複輸入
  - **討論欄位：** 影像結果、病理結果、討論要點、結論與建議、後續追蹤計畫
  - **手動加入病人（搜尋Modal）：** 姓名或病歷號搜尋、選擇加入哪個Section
  - **自動建立事件卡：** 手動加入必要提報/醫療小組時，同步幫病人新增事件卡
  - **結論寫回：** 完成會議時，有結論的病人自動寫回 mdt 事件卡
  - **必要提報類別chips：** 系統自動偵測的類別有⚡標記，可手動增減
  - 進度追蹤：Header 顯示 N/M 已有結論

### V6.5.5 (2026-04-10)
- **Phase 2：MDT 多專科會議管理頁（行事曆 + 建立會議）**
  - 新增「多專科會議」頁面（Sidebar + BottomNav 皆可進入）
  - 月份行事曆：
    - 實色徽章：已建立的會議（可點擊查看詳情）
    - 虛線徽章：系統根據排程原則自動推算的建議日期（點擊直接建立）
    - 7種癌別群組各有專屬顏色
  - 建立會議 Modal：
    - 選擇癌別群組 → 自動帶入排程原則、負責個管師、預設時間
    - 日期可手動修改（符合實際借會議室/醫師調整的彈性）
    - 時間二選一（07:30-08:30 / 12:30-13:30）
    - 地點、Zoom ID/密碼（選填）
  - 歷史記錄 tab：依月份分組，顯示所有已建立的會議
  - 點選歷史記錄跳回行事曆對應月份並展開詳情

### V6.5.4 (2026-04-10)
- **Phase 1：MDT 整合基礎建設**
  - 新增第 14 個癌別：LY 血液淋巴癌（Ann Arbor 分期，個管師：楊靜雯）
  - Event 介面新增 MDT 整合欄位：requiresMDT / mdtDiscussed / mdtMeetingId
  - DB v6：新增 meetings + meetingCases 兩張表（MDT 會議基礎架構）
  - 新增 Meeting / MeetingCase TypeScript 介面
  - 新增 MDT_GROUP_CONFIG（7個癌別群組 × 排程/個管師/時間）
  - 新增 CANCER_CODE_TO_MDT_GROUP / CANCER_CODE_TO_MANAGER 對應表
  - 新增 MANDATORY_CATEGORIES（基準2.4八類優先討論個案）
  - 多重癌偵測：同病歷號有多筆記錄時，病人清單兩筆都顯示「多重癌」tag
  - LY 癌別徽章顏色：紫色系（rgba(180,120,220)）
  - 所有 CANCER_CODE_MAP 同步更新加入 LY

### V6.5.3 (2026-04-05)
- **資料匯出改為預覽後匯出**
  - 點選任一匯出項目 → 下方展開該報表的網頁預覽
  - 預覽區右上角有「下載 CSV/TXT」按鈕才執行實際匯出
  - 指標達成率報表：表格顯示所有指標 · 分母 · 分子 · 達成率 · 達標/未達標
  - 指標明細：依癌別展開各指標，顯示未達標病人名單
  - 缺項清單：表格顯示每位病人缺漏的欄位（最多50筆預覽）
  - 病人清單：表格顯示病歷號 · 姓名 · 性別 · 癌別 · 初診日 · 分期 · 狀態
  - 評鑑摘要：顯示統計摘要 · 指標達成數 · 主要問題清單
  - 點擊已展開的項目 → 收合預覽
  - 選中的項目：左側藍線 + 背景色標示

### V6.5.2 (2026-04-05)
- **資料匯出頁全面強化**：
  - **頂部統計摘要**：收案範圍、收案人數、有效指標數、已達標數、整體達成率 + 進度條
  - **各癌別達成率 mini 條狀圖**：由低到高排列，顏色對應達成狀況（綠/黃/紅）
  - **匯出按鈕色彩系統**：每種匯出類型有專屬顏色（藍/紫/黃/綠/紅）+ loading spinner
  - **一鍵匯出**：同時下載達成率 + 缺項 + 病人清單三份常用報表
  - 修復：移除 ExportTab 函式中殘留的舊版 handler 和 JSX（~5,100 chars dead code）
...
- **全新測試資料（140位病人，每癌10人，橫跨 2024-2026）**
  - 13 個癌別 × 10 人 = 130 位指標病人 + 10 位工作中心觸發病人
  - 每個指標的分子✅/分母✅/分母無分子✅/不在分母✅ 四種情境均有覆蓋
  - 日期橫跨 2024-2026：約 3-4 人在 2026、3-4 人在 2025、2-3 人在 2024
  - 各癌別覆蓋設計：
    BC: Stage I-II-III-IV × HER2+/- × 哨兵/全切/保乳 × RT/anti-HER2
    CC: CIN3→錐狀、IA2+手術骨盆腔淋巴足/不足、放療首治×近接/無近接
    CRC: malignant polyp × 追加/不追加; 結腸手術淋巴足/不足; 直腸治療及時/延遲
    EC: R0/R1 × 淋巴足/不足 × 前導CRT/無 × 術後死亡
    EMC: 第一型/第二型 × IA/IB/II/III/IV × 術前影像/無 × TCGA標記/無
    GC: Stage I-IIIC × R0/R1 × 輔助化療/無 × ESD+胃鏡追蹤/無
    HCC: BCLC 0/A/B/C × RFA/TACE/手術 × 3個月影像追蹤/無 × 45天內/超
    LC: NSCLC/SCLC × IB-II-IIIA × 縱膈腔站數足/不足
    OC: 手術 × 6週內RT/延遲 × CCRT/RT only × 頸廓清淋巴足/不足 × 切緣
    OVC: 上皮性/非上皮性 × 術前CT/無 × R0/R1 × 殘存記錄/無
    PAC: Stage I-IV × 病理/無 × MDT/無 × 30天內治療 × R0/R1 × 輔助/無
    PC: 低風險AS × DRE/無 × PSA/無 × 局部侵犯RT-BED/根除術
    BLC: TURBT固有肌肉層/無 × 根除術骨盆腔淋巴足/不足

### V6.4.9 (2026-04-05)
- **Q1 README & CLAUDE.md 版本資訊更新**
- **Q2 修復：收案區間篩選器只在指標總覽看得到**
  - 原因：DateFilterButton 有 `{mainView !== 'data' && ...}` 的條件，只在非「待補項目」tab 顯示
  - 修復：移除條件，所有 tab 都顯示收案區間篩選器
  - 影響：待補項目、指標總覽、達成報告、趨勢預警、資料匯出 五個 tab 均有收案區間
- **Q3 修復：切換 2025 年只剩肝癌（只顯示有病人的癌別）**
  - 原因：summaryAggregatedData 只在 filteredQmPatients 中出現的癌別才建立 entry
  - 修復：pre-populate 所有 13 個癌別的空結構（分子/分母=0），再依有資料的病人累加
  - 結果：無論選哪個年份，指標總覽都顯示全部 13 個癌別，有資料則顯示數字，無資料顯示「無資料」
...
- **🔴 修復：改變區間後收案人數變但指標達成率沒跟著變**
  - 根本原因：資料流不一致
    - filteredQmPatients (有日期篩選) → missingList, nationalAggregatedData, summaryAggregatedData ✅
    - patientIndicators (loadData一次性載入，無日期篩選) → filteredIndicators → todoCategories → totalTodo ❌
  - 修復：filteredIndicators useMemo 加入 dateRange 篩選，與 filteredQmPatients 一致
  - 完整資料流現在全部通過 dateRange 篩選：
    totalTodo / 待補項目 / 缺漏清單 / 指標總覽 全部同步
...
- **全新測試資料設計（39位病人，確保每個指標分母非零）**
  - 逐一對應 60 個指標的分母條件，每個指標至少有一位病人在分母
  - 所有 initialDiagnosisDate 最大偏移 d(-88) = Jan 7, 2026，確保落在 2026 年內
  - 分母設計說明：
    BC: bc01(BC-2), bc02(BC-3), bc03(BC-4)
    CC: cc01(CC-1), cc02(CC-2+CC-5), cc03(CC-3+CC-4)
    CRC: crc01(CRC-1), crc02(CRC-2), crc03(CRC-3)
    EC: ec01(EC-1+2+3+4)
    EMC: emc01(EMC-1+2+8), emc02(EMC-3+5), emc03(EMC-4+6+7)
    GC: gc01(GC-1+3+4), gc02(GC-5)
    HCC: hcc01(HCC-1+2+4), hcc02(HCC-5), hcc03(HCC-6)
    LC: lc01(LC-1), lc02(LC-2)
    OC: oc01(OC-1+2+5+6), oc02(OC-3+4)
    OVC: ovc01(OVC-1+2+3)
    PAC: pac01(PAC-1+2+3+4+5+6+7), pac02(PAC-8+9)
    PC: pc01(PC-1+2+3), pc02(PC-4), pc03(PC-5)
    BLC: blc01(BLC-1+2+3)
  - 工作中心觸發病人 10 位（全7種觸發類型覆蓋）

### V6.4.4 (2026-04-05)
- **🔴 修復：指標總覽大多數癌別顯示「無資料」**
  - 根本原因：nationalAggregatedData 使用 filteredQmPatients（套用年度篩選器），預設為 year:2026
  - 測試病人 initialDiagnosisDate 使用 d(-180) 等大偏移量 → 落在 2025 → 被日期過濾器排除
  - 雙重修復：
    1. 指標總覽改用 summaryAggregatedData（基於 qmPatients，不受日期篩選影響）
    2. 測試資料所有日期偏移量壓縮至 2026 年內（最大 d(-90) = Jan 5, 2026）
  - WC08 失聯病人保留 d(-400) 確保仍觸發失聯判斷（超過12個月）

### V6.4.3 (2026-04-05)
- **Q1 修復：指標缺漏展開後 → 按鈕無作用**
  - 根本原因：onNavigateToPatient 的 callback 主體為空 `if (pi) { }`
  - 修復：改為直接 `navigateToPatient(id, 'indicators')`
  - 同樣適用依指標視圖中每個病人整行點擊
- **Q2 新增：指標總覽 tab（第2個 tab）**
  - 顯示每個癌別所有指標的達成情況，預設全部收合
  - 每個癌別 header：癌別名稱 + 達成進度條 + N/M 達成 + 百分比
  - 展開後每個指標顯示：指標 ID + 名稱 + 分子/分母 + 百分比 + 達成狀態圓點
  - 無資料時顯示「無資料」，有資料時用綠/紅色區分達標/未達標
